...
void _TCS3472_getRGB(char buf[8], MicroBitI2C *i2c) {
	buf[0] = 0xA0 | 0x14;
	...
	buf[0] = ((unsigned) buf[3] << 8 | (unsigned) buf[2]) * scale;
	buf[1] = ((unsigned) buf[5] << 8 | (unsigned) buf[4]) * scale;
	buf[2] = ((unsigned) buf[7] << 8 | (unsigned) buf[6]) * scale;
}

uint8_t _TCS3472_getRed(char buf[8], MicroBitI2C *i2c) {
	_TCS3472_getRGB(buf, i2c);
	return buf[0];
}

uint8_t _TCS3472_getGreen(char buf[8], MicroBitI2C *i2c) {
	_TCS3472_getRGB(buf, i2c);
	return buf[1];
}

uint8_t _TCS3472_getBlue(char buf[8], MicroBitI2C *i2c) {
	_TCS3472_getRGB(buf, i2c);
	return buf[2];
}

uint8_t _TCS3472_getLight(char buf[8], MicroBitI2C *i2c) {
	_TCS3472_getRGB(buf, i2c);
	return ((buf[0] * 1.13 + buf[1] * 1.53 + buf[2] * 1.82) * 0.13);
}

void _TCS3472_init(char buf[8], MicroBitI2C *i2c) {
	// enable and power on
	buf[0] = 0xA0 | 0x00;
	i2c->write( _TCS3472_ADDR, buf, 1, true);
	...
	_TCS3472_setIntegrationTime(buf, i2c);
	_TCS3472_setGain(buf, i2c);
}
